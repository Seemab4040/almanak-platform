...
// --- FORM HELPERS ---
const inputCls = "w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-[#0f2537] focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-all";
...
const NavItem = ({ icon, label, active, onClick }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-all duration-200 rounded-lg group ${active ? 'bg-gradient-to-r from-amber-50 to-amber-100/50 text-amber-900 border-l-4 border-[#c49b66]' : 'text-slate-600 hover:bg-[#faf8f5] hover:text-[#1a3a52]'}`}>
    <span className={`text-lg transition-colors ${active ? 'text-[#c49b66]' : 'text-slate-400 group-hover:text-slate-600'}`}>{icon}</span>
    ...
    <aside className={`w-64 border-r flex flex-col h-screen shrink-0 transition-colors duration-300 ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
      <div className="p-5 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div><div className="font-bold text-[#1a3a52] text-sm tracking-tight">Al-Malak</div><div className="text-xs text-slate-400">BY SAHWAN LAW</div></div>
        </div>
      </div>
      <div className={`p-3 border-b transition-colors ${darkMode ? 'border-slate-700' : 'border-slate-100'} relative`}>
        ...
        <div onClick={() => setShowAssocDropdown(!showAssocDropdown)} className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors ${darkMode ? 'bg-slate-700 hover:bg-slate-600' : 'bg-[#faf8f5] hover:bg-slate-100'}`}>
          ...
          <div className="flex-1 min-w-0">
            <div className={`text-xs font-semibold truncate ${darkMode ? 'text-slate-200' : 'text-slate-700'}`}>{selectedAssociation?.name || 'Select Association'}</div>
        ...
        {showAssocDropdown && (
          <div className="absolute top-full left-3 right-3 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-40 overflow-hidden max-h-96 overflow-y-auto">
              ...
              <div className="p-2">
                ...
                {userAssociations.map((userAssoc) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`w-6 h-6 rounded flex items-center justify-center text-xs font-bold ${isSelected ? 'bg-amber-500 text-white' : 'bg-slate-200 text-slate-600'}`}>
                        {assoc.name.substring(0, 2).toUpperCase()}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-semibold text-slate-700 truncate">{assoc.name}</div>
            ...
            {selectedAssociation && (
              <div className="border-t border-slate-100 p-3 bg-[#faf8f5]">
                <div className="text-xs font-bold text-slate-500 uppercase mb-2">Current Association Info</div>
                <div className="flex justify-between text-xs"><span className="text-slate-500">Total Units</span><span className="font-semibold text-slate-700">{selectedAssociation.total_units}</span></div>
                <div className="flex justify-between text-xs mt-1"><span className="text-slate-500">Registered Owners</span><span className="font-semibold text-slate-700">{selectedAssociation.registered_owners}</span></div>
                <div className="flex justify-between text-xs mt-1"><span className="text-slate-500">Active Since</span><span className="font-semibold text-slate-700">{selectedAssociation.active_since}</span></div>
              </div>
            )}
      ...
      <div className="flex items-center gap-3">
            ...
            <div className="absolute top-7 right-0 w-80 bg-white border border-slate-200 rounded-xl shadow-xl z-50">
              ...
              {notifications.map(n => (
                <div key={n.id} className={`flex items-start gap-3 p-3 hover:bg-[#faf8f5] cursor-pointer transition-colors ${!n.read ? "bg-blue-50/40" : ""}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className={`text-xs ${!n.read ? "font-semibold text-[#0f2537]" : "text-slate-600"}`}>{n.title}</div>
              ...
              <div className="p-3 border-t border-slate-100 text-center"><span onClick={() => setShowBell(false)} className="text-xs text-[#c49b66] font-semibold cursor-pointer hover:underline">View all notifications</span></div>
            </div>
          )}
        </div>
        {/* Settings */}
        <div className="relative">
          <div onClick={() => setShowSettings(!showSettings)} className="cursor-pointer">
            <Settings size={20} className={`transition-colors ${showSettings ? "text-[#c49b66]" : "text-slate-500 hover:text-slate-700"}`} />
          </div>
          {showSettings && (
                ...
                {!showLogoutConfirm ? (
                  ...
                  <div className="border-t border-slate-100 mt-2 pt-3 space-y-2">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => setShowLogoutConfirm(false)} className="flex-1 text-xs text-slate-600 font-semibold py-1.5 rounded-lg border border-slate-200 hover:bg-[#faf8f5]">Cancel</button>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
          ...
          {legalCases.map(c => (
            <div key={c.id} className="p-3 hover:bg-[#faf8f5] rounded-lg transition-colors">
              ...
              <div className="flex items-center gap-1 mt-1.5 text-xs text-[#b8895a] font-medium"><ArrowRight size={11} /><span>Next Step: </span><span className="text-slate-700">{c.next_step}</span></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mt-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {complianceItems.map(item => (
            <div key={item.id} className={`p-3 rounded-lg border ${item.status === 'expired' || item.priority === 'critical' ? 'border-red-200 bg-red-50' : item.status === 'completed' ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-[#faf8f5]'}`}>
    ...
    <div className="animate-fade-in">
      ...
      <Modal open={showMinutesModal} onClose={() => { setShowMinutesModal(false); setSelectedMeeting(null); setEditingMinutes(null); }} title={editingMinutes ? "Edit Minutes of Meeting" : "Add Minutes of Meeting"}>
        {selectedMeeting && (
          <form onSubmit={handleSaveMinutes} className="space-y-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Attendees *</label>
              <textarea 
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Topics Discussed *</label>
              <textarea 
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Decisions Made *</label>
              <textarea 
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Action Items *</label>
              <textarea 
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Next Meeting</label>
              <input 
                type="text"
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <div className="relative">
              <button onClick={() => setShowFilter(!showFilter)} className={`flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg transition-colors ${showFilter || filterType !== 'all' || filterStatus !== 'all' ? 'bg-[#f5ede3] text-[#b8895a]' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
        ...
        <table className="w-full text-sm">
          <thead><tr className="bg-[#faf8f5] border-b border-slate-100">
    ...
    <div className="animate-fade-in">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <div className="relative">
              <button onClick={() => setShowFilter(!showFilter)} className={`flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg transition-colors ${showFilter || filterDays !== 'all' || filterStatus !== 'all' ? 'bg-[#f5ede3] text-[#b8895a]' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>{filteredUnits.map(row => (
              <tr key={row.id} className="border-b border-slate-50 hover:bg-[#faf8f5] transition-colors">
                ...
                <td className="p-3 text-slate-600 text-xs">{row.collection || '-'}</td>
                <td className="p-3 text-slate-600 text-xs">{row.unprocess || '-'}</td>
                <td className="p-3">
                  <div className="flex gap-1">
    ...
    <div className="animate-fade-in">
      ...
      <Modal open={!!updateModal} onClose={() => setUpdateModal(null)} title="Update Compliance Item">
        {updateModal && (
          <form onSubmit={handleUpdate} className="space-y-3">
            <div className="bg-[#faf8f5] rounded-lg p-3"><div className="text-sm font-semibold text-slate-700">{updateModal.item}</div><div className="text-xs text-slate-400">Deadline: {updateModal.deadline}</div></div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <div className="relative">
              <button onClick={() => setShowFilter(!showFilter)} className={`flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg transition-colors ${showFilter || filterPriority !== 'all' || filterStatus !== 'all' ? 'bg-[#f5ede3] text-[#b8895a]' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex gap-2 mb-6 border-b border-slate-200">
        <button 
          ...
          className={`px-4 py-2 font-semibold text-sm transition-colors ${activeTab === 'overview' ? 'border-b-2 border-[#c49b66] text-[#c49b66]' : 'text-slate-600 hover:text-[#1a3a52]'}`}
        >
          Overview
        </button>
        <button 
          onClick={() => setActiveTab('collection')}
          className={`px-4 py-2 font-semibold text-sm transition-colors ${activeTab === 'collection' ? 'border-b-2 border-[#c49b66] text-[#c49b66]' : 'text-slate-600 hover:text-[#1a3a52]'}`}
        >
          Collection Service
        </button>
        <button 
          onClick={() => setActiveTab('legal')}
          className={`px-4 py-2 font-semibold text-sm transition-colors ${activeTab === 'legal' ? 'border-b-2 border-[#c49b66] text-[#c49b66]' : 'text-slate-600 hover:text-[#1a3a52]'}`}
        >
          Legal Service
        </button>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div 
          onClick={() => setActiveTab('collection')}
          className="bg-white rounded-xl border border-slate-200 p-6 cursor-pointer hover:shadow-lg transition-all"
        >
          ...
          <tbody>
            {collectionRecords.map(record => (
              <tr key={record.id} className="border-b border-slate-50 hover:bg-[#faf8f5]">
                ...
                <td className="p-3 text-slate-600">{record.owner || '-'}</td>
                <td className="p-3 font-semibold text-emerald-700">BD {Number(record.amount_received).toLocaleString()}</td>
                <td className="p-3 text-slate-600">{record.settlement ? `BD ${Number(record.settlement).toLocaleString()}` : '-'}</td>
                <td className="p-3 text-slate-600 text-xs">{record.date}</td>
                <td className="p-3">
                  <div className="flex gap-2">
        ...
        <div className="divide-y divide-slate-100">
          {legalCases.map(c => (
            <div key={c.id} className="p-4 hover:bg-[#faf8f5] transition-colors">
              ...
              {c.next_action && (
                <div className="mt-2 text-sm"><span className="text-slate-500">Last Action:</span> <span className="text-slate-700">{c.last_action || 'N/A'}</span></div>
              )}
              {c.next_action && (
                <div className="mt-1 flex items-center gap-2 text-sm"><span className="text-[#c49b66]">→ Next:</span><span className="font-medium text-slate-700">{c.next_action}</span></div>
              )}
            </div>
          ))}
        </div>
    ...
    <div className="animate-fade-in">
      ...
      <Modal open={showDocModal || !!editingDoc} onClose={() => { setShowDocModal(false); setEditingDoc(null); setDocForm({ doc_type: 'Memorandum', file_name: '', uploaded_date: new Date().toISOString().split('T')[0], status: 'pending' }); setSelectedFile(null); }} title={editingDoc ? "Edit Document" : "Add Required Document"}>
        <form onSubmit={editingDoc ? handleEditDoc : handleAddDoc} className="space-y-3">
          ...
          {/* File Upload Section */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1.5">Upload File (PDF, DOC, DOCX)</label>
            <div className="relative">
              <input 
                type="file" 
                accept=".pdf,.doc,.docx" 
          ...
          {filteredComplaints.map(c => (
            <div key={c.id} className="p-4 hover:bg-[#faf8f5] transition-colors">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className={`text-xs font-bold px-2 py-1 rounded-lg border ${c.priority === 'critical' ? 'bg-red-50 text-red-700 border-red-200' : c.priority === 'high' ? 'bg-orange-50 text-orange-700 border-orange-200' : 'bg-[#faf8f5] text-slate-600 border-slate-200'}`}>{c.unit}</span>
            ...
            docs.length > 0 && (
              <div key={type} className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-semibold text-slate-700 text-sm">{type}</h4>
                  <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{docs.length}</span>
                </div>
                <div className="space-y-2 ml-6">
                  {docs.map(doc => (
                    <div key={doc.id} className="flex items-center justify-between p-3 bg-[#faf8f5] rounded-lg hover:bg-slate-100 transition-colors">
                      <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => setEditingDoc(doc)} className="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded hover:bg-slate-200">Edit</button>
    ...
    <div className="animate-fade-in">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mb-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {boardMembers.map(member => (
            <div key={member.id} className="border border-slate-200 rounded-xl p-4 hover:shadow-md transition-all relative group">
              ...
              <div className="flex items-center gap-3 mb-3"><div className="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center text-xl shadow-inner">🏢</div><div><div className="font-semibold text-[#0f2537] text-sm">{member.name}</div><div className="text-xs text-slate-500">{member.role}</div></div></div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mb-6">
        ...
        {lawyers.length > 0 ? (
          <div className="space-y-2">
            {lawyers.map(lawyer => (
              <div key={lawyer.id} className="flex items-center justify-between p-3 bg-[#faf8f5] rounded-lg hover:bg-slate-100 transition-colors">
                <div className="flex-1">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mb-6">
        ...
        {vendors.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="bg-[#faf8f5] border-b border-slate-200">
              ...
              <tbody>
                {vendors.map(vendor => (
                  <tr key={vendor.id} className="border-b border-slate-50 hover:bg-[#faf8f5] transition-colors">
                    ...
                    <td className="p-3 text-slate-600">{vendor.office_no}</td>
                    <td className="p-3 text-slate-600">{vendor.email}</td>
                    <td className="p-3 text-slate-600">{vendor.mobile_no}</td>
                    <td className="p-3 text-slate-600">{vendor.service_type}</td>
                    <td className="p-3">
                      <div className="flex gap-2">
  ...
  return (
    <>
      {!isAuthenticated ? (
        ...
        <div className={`min-h-screen flex text-[#1a3a52] font-sans transition-colors duration-300 ${darkMode ? 'bg-[#1a3a52]' : 'bg-[#faf8f5]'}`}>
