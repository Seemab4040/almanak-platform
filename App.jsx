              ...
              <div className="p-4 space-y-4">
                <div>
                  ...
                  <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
                    ...
                    <div><div className="text-sm font-semibold text-slate-800">Prof. Alex Atanassoff</div><div className="text-xs text-slate-500">Chairman — Lagoon Heights</div></div>
                  </div>
                </div>
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between p-2 hover:bg-slate-50 rounded-lg">
                      ...
                      <div onClick={() => setToggleDark(!toggleDark)} className={`w-9 h-5 rounded-full relative cursor-pointer transition-colors duration-200 ${toggleDark ? 'bg-emerald-500' : 'bg-slate-300'}`}><div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-all duration-200 ${toggleDark ? 'left-4' : 'left-0.5'}`}></div></div>
                    </div>
                  </div>
                </div>
                <div>
                ...
                {!showLogoutConfirm ? (
                  ...
                  <div className="border-t border-slate-100 mt-2 pt-3 space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => { setShowSettings(false); setShowLogoutConfirm(false); alert('Logged out successfully. Redirecting...'); }} className="flex-1 text-xs text-white font-semibold py-1.5 rounded-lg bg-red-500 hover:bg-red-600">Log Out</button>
                    </div>
                  </div>
                )}
              </div>
...
const Meetings = ({ meetings, setMeetings }) => {
  ...
  return (
    <div className="animate-fade-in">
      <NewMeetingModal open={showModal} onClose={() => setShowModal(false)} onAdd={(rows) => setMeetings(prev => [...prev, ...rows])} />
...
const Compliance = ({ complianceItems, setComplianceItems }) => {
  ...
  const handleAdd = async (e) => {
    ...
    try { const r = await postToSupabase('compliance_items', form); setComplianceItems(prev => [...prev, ...r]); setShowModal(false); setForm({ item: '', status: 'pending', deadline: '', priority: 'medium' }); }
    catch (err) { alert(err.message); } finally { setSaving(false); }
  };
  const handleUpdate = async (e) => {
    e.preventDefault(); setSaving(true);
    try { await patchToSupabase('compliance_items', updateModal.id, { status: updateModal.status, priority: updateModal.priority }); setComplianceItems(prev => prev.map(i => i.id === updateModal.id ? { ...i, status: updateModal.status, priority: updateModal.priority } : i)); setUpdateModal(null); }
    catch (err) { alert(err.message); } finally { setSaving(false); }
  };
  return (
    <div className="animate-fade-in">
...
const Complaints = ({ complaints, setComplaints }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try { const r = await postToSupabase('complaints', form); setComplaints(prev => [...prev, ...r]); setShowModal(false); setForm({ unit: '', subject: '', date: new Date().toISOString().split('T')[0], status: 'pending', priority: 'medium' }); }
    catch (err) { alert(err.message); } finally { setSaving(false); }
  };
  return (
    <div className="animate-fade-in">
...
const Board = ({ boardMembers, setBoardMembers, documents, setDocuments }) => {
  ...
  const handleAddMember = async (e) => {
    ...
    try { const r = await postToSupabase('board_members', memberForm); setBoardMembers(prev => [...prev, ...r]); setShowMemberModal(false); setMemberForm({ name: '', role: 'Member', status: 'pending_registration', since: new Date().toISOString().split('T')[0] }); }
    catch (err) { alert(err.message); } finally { setSaving(false); }
  };
  const handleAddDoc = async (e) => {
    e.preventDefault(); if (!docForm.name) return; setSaving(true);
    try { const r = await postToSupabase('documents', docForm); setDocuments(prev => [...prev, ...r]); setShowDocModal(false); setDocForm({ name: '', date: new Date().toISOString().split('T')[0], status: 'current' }); }
    catch (err) { alert(err.message); } finally { setSaving(false); }
  };
  return (
    <div className="animate-fade-in">
