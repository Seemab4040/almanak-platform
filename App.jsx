...
import { AlertCircle, AlertTriangle, ArrowRight, BadgeDollarSign, Bell, Building, Calendar, Check, ChevronDown, ChevronRight, Circle, ClipboardCheck, Download, ExternalLink, FileText, Filter, Gavel, LayoutDashboard, MessageSquarePlus, MessageSquareWarning, Plus, Scale, Search, Settings, ShieldCheck, TrendingDown, TrendingUp, Users } from 'lucide-react';
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 bg-white border-r border-slate-200 shadow-sm z-50 flex flex-col">
      ...
      <nav className="p-3 space-y-1 flex-1 overflow-y-auto">
        <NavItem id="dashboard" icon={<LayoutDashboard size={20} />} label="Dashboard" active={activeSection === 'dashboard'} onClick={() => setActiveSection('dashboard')} />
        ...
        <NavItem id="financial" icon={<BadgeDollarSign size={20} />} label="Financial" active={activeSection === 'financial'} onClick={() => setActiveSection('financial')} />
        <NavItem id="legal" icon={<Scale size={20} />} label="Legal Services" active={activeSection === 'legal'} onClick={() => setActiveSection('legal')} />
        <NavItem id="complaints" icon={<MessageSquareWarning size={20} />} label="Complaints" active={activeSection === 'complaints'} onClick={() => setActiveSection('complaints')} />
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        ...
        <button className="px-4 py-2 bg-amber-600 text-white rounded-lg font-medium hover:bg-amber-700 transition-colors shadow-sm flex items-center gap-2">
          <MessageSquarePlus size={18} /> New Complaint
        </button>
      </div>
