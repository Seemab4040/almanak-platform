  ...
  return (
    <aside className={`w-64 border-r flex flex-col h-screen shrink-0 transition-colors duration-300 ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
      ...
      <div className={`p-3 border-b transition-colors ${darkMode ? 'border-slate-700' : 'border-slate-100'} relative`}>
        ...
        <div onClick={() => setShowAssocDropdown(!showAssocDropdown)} className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors ${darkMode ? 'bg-slate-700 hover:bg-slate-600' : 'bg-slate-50 hover:bg-slate-100'}`}>
                ...
                {userAssociations.map((userAssoc) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors ${isSelected ? 'bg-amber-50 border border-amber-200' : 'hover:bg-slate-50'}`}
                    >
        ...
        {selectedMeeting && (
          <form onSubmit={handleSaveMinutes} className="space-y-3">
            <div className="bg-slate-50 p-3 rounded-lg mb-3">
              <div className="text-sm font-semibold text-slate-800">{selectedMeeting.type}</div>
            ...
            <div>
              ...
              <textarea 
                ...
                className="w-full text-sm border border-slate-300 rounded-lg px-3 py-2 min-h-[60px] focus:outline-none focus:ring-2 focus:ring-amber-500"
                required
              />
            </div>
            ...
            <div>
              ...
              <textarea 
                ...
                className="w-full text-sm border border-slate-300 rounded-lg px-3 py-2 min-h-[80px] focus:outline-none focus:ring-2 focus:ring-amber-500"
                required
              />
            </div>
              ...
              <td className="p-3">
                <div className="flex gap-1">
                  {getMinutesStatus(m.id) ? (
                    ...
                    <button onClick={() => openMinutesModal(m)} className="text-xs text-amber-600 font-semibold hover:underline">Add Minutes</button>
                  )}
                </div>
              </td>
            </tr>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowAddModal(true)} className="flex items-center gap-1 text-xs bg-amber-600 text-white px-3 py-1.5 rounded-lg hover:bg-amber-700"><Plus size={13} /> Add Unit</button>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>{filteredUnits.map(row => (
              <tr key={row.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                ...
                <td className="p-3">
                  <div className="flex gap-1">
                    <button onClick={() => setNoticeUnit(row)} className="text-xs text-amber-600 font-semibold hover:underline">Notice</button>
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex gap-2 mb-6 border-b border-slate-200">
        <button 
          ...
          className={`px-4 py-2 font-semibold text-sm transition-colors ${activeTab === 'overview' ? 'border-b-2 border-amber-600 text-amber-600' : 'text-slate-600 hover:text-slate-900'}`}
        >
          Overview
        </button>
        <button 
          onClick={() => setActiveTab('collection')}
          className={`px-4 py-2 font-semibold text-sm transition-colors ${activeTab === 'collection' ? 'border-b-2 border-amber-600 text-amber-600' : 'text-slate-600 hover:text-slate-900'}`}
        >
          Collection Service
        </button>
        <button 
          onClick={() => setActiveTab('legal')}
          className={`px-4 py-2 font-semibold text-sm transition-colors ${activeTab === 'legal' ? 'border-b-2 border-amber-600 text-amber-600' : 'text-slate-600 hover:text-slate-900'}`}
        >
          Legal Service
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h3 className="font-semibold text-slate-800">Collection Service</h3>
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h3 className="font-semibold text-slate-800">Legal Service</h3>
...
const CollectionService = ({ collectionRecords, setCollectionRecords, selectedAssociation }) => {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          <h3 className="font-semibold text-slate-800">Collection Records</h3>
          <button onClick={() => setShowModal(true)} className="flex items-center gap-1 text-xs bg-amber-600 text-white px-3 py-1.5 rounded-lg hover:bg-amber-700"><Plus size={13} /> Add Record</button>
        </div>
        <table className="w-full text-sm">
          <thead><tr className="bg-slate-50 border-b border-slate-100">
            <th className="text-left p-3 text-xs font-semibold text-slate-500 uppercase">Unit</th>
          ...
          <tbody>
            {collectionRecords.map(record => (
              <tr key={record.id} className="border-b border-slate-50 hover:bg-slate-50">
                <td className="p-3 font-medium text-slate-800">{record.unit}</td>
            ...
            <div key={c.id} className="p-4 hover:bg-slate-50 transition-colors">
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1"><span className="text-lg">{c.status === 'investigation' ? '🔍' : c.status === 'notice_sent' ? '📧' : c.status === 'in_court' ? '⚖️' : '✅'}</span><h4 className="font-semibold text-slate-800">{c.title}</h4><StatusBadge status={c.status.replace('_', ' ')} /></div>
              ...
              {c.next_action && (
                <div className="mt-1 flex items-center gap-2 text-sm"><span className="text-amber-600">→ Next:</span><span className="font-medium text-slate-700">{c.next_action}</span></div>
              )}
            </div>
          ))}
        </div>
    ...
    <div className="animate-fade-in">
          ...
          <div>
            ...
            <div className="relative">
              <input 
                ...
                className="w-full text-xs border border-slate-300 rounded-lg px-3 py-2 bg-white file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100 cursor-pointer"
              />
            </div>
            {selectedFile && (
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <button onClick={() => setShowDocModal(true)} className="flex items-center gap-1 text-xs bg-amber-600 text-white px-3 py-1.5 rounded-lg hover:bg-amber-700"><Plus size={13} /> Add Document</button>
        </div>
                ...
                <div className="space-y-2 ml-6">
                  {docs.map(doc => (
                    <div key={doc.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
                      <div className="flex-1">
                        <div className="font-medium text-slate-800 text-sm">{doc.file_name}</div>
...
const Board = ({ boardMembers, setBoardMembers, documents, setDocuments, selectedAssociation }) => {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-slate-800">Property Management</h3>
          <button onClick={() => setShowMgrModal(true)} className="flex items-center gap-1 text-xs bg-amber-600 text-white px-3 py-1.5 rounded-lg hover:bg-amber-700"><Plus size={13} /> Add Manager</button>
        </div>
        {managers.length > 0 ? managers.map(mgr => (
          <div key={mgr.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl mb-2 relative group">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-xl bg-white flex items-center justify-center text-2xl shadow border">🏢</div>
              <div><div className="font-semibold text-slate-800">{mgr.name}</div><div className="text-sm text-slate-500 font-medium">Contact: {mgr.contact}</div><div className="text-xs text-slate-400">Services: {mgr.services}</div></div>
            </div>
            <div className="flex items-center gap-2">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-slate-800">Lawyers</h3>
          <button onClick={() => setShowLawyerModal(true)} className="flex items-center gap-1 text-xs bg-amber-600 text-white px-3 py-1.5 rounded-lg hover:bg-amber-700"><Plus size={13} /> Add Lawyer</button>
        </div>
        {lawyers.length > 0 ? (
          <div className="space-y-2">
            {lawyers.map(lawyer => (
              <div key={lawyer.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
                <div className="flex-1">
                  <div className="font-semibold text-slate-800 text-sm">{lawyer.name}</div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => setShowVendorModal(true)} className="flex items-center gap-1 text-xs bg-amber-600 text-white px-3 py-1.5 rounded-lg hover:bg-amber-700"><Plus size={13} /> Add Vendor</button>
        </div>
        {vendors.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {vendors.map(vendor => (
                  <tr key={vendor.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    <td className="p-3 font-medium text-slate-800">{vendor.name}</td>
