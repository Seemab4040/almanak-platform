...
const Board = ({ boardMembers, setBoardMembers, documents, setDocuments, selectedAssociation }) => {
  ...
  const handleAddMember = async (e) => {
    ...
    try { const r = await postToSupabase('board_members', { ...memberForm, association_id: selectedAssociation?.id }); setBoardMembers(prev => [...prev, ...r]); setShowMemberModal(false); showToast('Board member added successfully', 'success'); setMemberForm({ name: '', role: 'Member', status: 'pending_registration', since: new Date().toISOString().split('T')[0] }); }
    catch (err) { alert(err.message); } finally { setSaving(false); }
  };
  const handleAddDoc = async (e) => {
    e.preventDefault(); if (!docForm.name) return; setSaving(true);
    try { const r = await postToSupabase('documents', { ...docForm, association_id: selectedAssociation?.id }); setDocuments(prev => [...prev, ...r]); setShowDocModal(false); showToast('Document added successfully', 'success'); setDocForm({ name: '', date: new Date().toISOString().split('T')[0], status: 'current' }); }
    catch (err) { alert(err.message); } finally { setSaving(false); }
  };
  return (
    <div className="animate-fade-in">
