...
const Complaints = ({ complaints, setComplaints, selectedAssociation }) => {
  ...
  const handleAddDoc = async (e) => {
    e.preventDefault(); if (!docForm.file_name) return; setSaving(true);
    try {
      const r = await postToSupabase('required_documents', { ...docForm, association_id: selectedAssociation?.id });
  ...
  const handleEditDoc = async (e) => {
    e.preventDefault(); setSaving(true);
    try {
      await patchToSupabase('required_documents', editingDoc.id, editingDoc);
      setRequiredDocs(prev => prev.map(d => d.id === editingDoc.id ? editingDoc : d));
      setEditingDoc(null);
  ...
  const handleDocumentDownload = (doc) => {
    const blob = new Blob([`Document Type: ${doc.doc_type}\nFile: ${doc.file_name}\nUploaded: ${doc.uploaded_date}\nStatus: ${doc.status}\n\nThis is a simulated document download.`], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `${doc.file_name.replace(/\s/g, "-")}.txt`;
    a.click();
    URL.revokeObjectURL(url);
  ...
  return (
    <div className="animate-fade-in">
      ...
      <Modal open={showDocModal || !!editingDoc} onClose={() => { setShowDocModal(false); setEditingDoc(null); setDocForm({ doc_type: 'Memorandum', file_name: '', uploaded_date: new Date().toISOString().split('T')[0], status: 'pending' }); }} title={editingDoc ? "Edit Document" : "Add Required Document"}>
        <form onSubmit={editingDoc ? handleEditDoc : handleAddDoc} className="space-y-3">
          ...
          <TextInput label="File Name" value={editingDoc?.file_name || docForm.file_name} onChange={v => editingDoc ? setEditingDoc({...editingDoc, file_name: v}) : setDocForm({...docForm, file_name: v})} placeholder="e.g. Memorandum-2026.pdf" />
