...
const TextInput = ({ label, value, onChange, placeholder, type }) => (
  <div>
    <label className={labelCls}>{label}</label>
    <input type={type || "text"} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className={inputCls} />
  </div>
);
...
const Header = ({ activeSection }) => {
  ...
  return (
    <header className="flex items-center justify-between mb-8">
      ...
      <div className="flex items-center gap-3">
        <div className="relative"><Bell size={20} className="text-slate-500 cursor-pointer hover:text-slate-700 transition-colors" /><span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full flex items-center justify-center"><span className="text-white text-xs font-bold">3</span></span></div>
        ...
        <div className="text-right"><div className="text-xs font-semibold text-slate-700">Prof. Alex</div><div className="text-xs text-slate-400">Chairman</div></div>
      </div>
    </header>
  );
};
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
          <div className="flex items-center justify-between mb-4"><div className="flex items-center gap-2"><span className="text-lg">📅</span><span className="font-semibold text-slate-800">Upcoming Meetings</span></div><span className="text-xs text-amber-600 font-semibold cursor-pointer hover:underline">Schedule New →</span></div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
          <div className="flex items-center justify-between mb-4"><div className="flex items-center gap-2"><span className="text-lg">⚖️</span><span className="font-semibold text-slate-800">Legal Activity</span></div><span className="text-xs text-amber-600 font-semibold cursor-pointer hover:underline">View All →</span></div>
          {legalCases.map(c => (
...
const Meetings = ({ meetings, setMeetings }) => {
  ...
  return (
    <div className="animate-fade-in">
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Schedule New Meeting">
...
const Financial = ({ delinquentUnits, setDelinquentUnits }) => {
  ...
  const [saving, setSaving] = useState(false);
  const totalOwing = delinquentUnits.reduce((s, u) => s + Number(u.amount), 0);
    ...
    <div className="animate-fade-in">
        ...
        {noticeUnit && (
          <div className="space-y-4">
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
              <div className="text-sm font-semibold text-amber-800">Notice Details</div>
              <div className="text-xs text-amber-600 mt-1">A formal payment notice will be sent to the owner.</div>
            </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <div className="flex items-center gap-2 mb-3"><span className="text-lg">💳</span><span className="font-semibold text-slate-700 text-sm">Payment Methods</span></div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <div className="flex items-center gap-2 mb-3"><span className="text-lg">📊</span><span className="font-semibold text-slate-700 text-sm">Budget Overview</span></div>
          <div className="space-y-2">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <div className="flex items-center gap-2 mb-3"><span className="text-lg">📈</span><span className="font-semibold text-slate-700 text-sm">Reserves</span></div>
          <div className="space-y-2">
...
const Compliance = ({ complianceItems, setComplianceItems }) => {
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    if (!form.item || !form.deadline) return;
    setSaving(true);
    try {
      const newRow = await postToSupabase('compliance_items', form);
      setComplianceItems(prev => [...prev, ...newRow]);
      setShowModal(false);
          ...
          {complianceItems.map(item => (
            <div key={item.id} className="p-4 hover:bg-slate-50 transition-colors flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setUpdateModal({...item})} className="text-xs text-amber-600 font-semibold hover:underline ml-2">Update</button>
              </div>
            </div>
          ))}
        </div>
...
const Legal = ({ legalCases, setLegalCases }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const newRow = await postToSupabase('legal_cases', payload);
      setLegalCases(prev => [...prev, ...newRow]);
      setShowModal(false);
      setForm({ type: 'Debt Collection', description: '', amount: '', status: 'pending', last_action: '', next_step: '' });
    } catch (err) { alert(err.message); }
    finally { setSaving(false); }
  };
            ...
            <div key={c.id} className="p-5 hover:bg-slate-50 transition-colors">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-slate-800 text-sm">{c.type}</span>
              ...
              <div className="ml-10 mt-2 space-y-1">
                <div className="text-xs text-slate-400"><span className="font-medium text-slate-600">Last Action:</span> {c.last_action}</div>
                <div className="text-xs text-amber-700 font-medium flex items-center gap-1"><ArrowRight size={11} /> Next: {c.next_step}</div>
              </div>
            </div>
          ))}
        </div>
...
const Complaints = ({ complaints, setComplaints }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.unit || !form.subject) return;
    setSaving(true);
    try {
      const newRow = await postToSupabase('complaints', form);
      setComplaints(prev => [...prev, ...newRow]);
      setShowModal(false);
      setForm({ unit: '', subject: '', date: new Date().toISOString().split('T')[0], status: 'pending', priority: 'medium' });
    } catch (err) { alert(err.message); }
    finally { setSaving(false); }
  };
...
const Board = ({ boardMembers, setBoardMembers, documents, setDocuments }) => {
  ...
  const handleAddMember = async (e) => {
    ...
    setSaving(true);
    try {
      const newRow = await postToSupabase('board_members', memberForm);
      setBoardMembers(prev => [...prev, ...newRow]);
  ...
  const handleAddDoc = async (e) => {
    ...
    setSaving(true);
    try {
      const newRow = await postToSupabase('documents', docForm);
      setDocuments(prev => [...prev, ...newRow]);
      setShowDocModal(false);
      setDocForm({ name: '', date: new Date().toISOString().split('T')[0], status: 'current' });
    } catch (err) { alert(err.message); }
    finally { setSaving(false); }
  };
          ...
          {boardMembers.map(member => (
            <div key={member.id} className="border border-slate-200 rounded-xl p-4 hover:shadow-md transition-all">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div><div className="font-semibold text-slate-800 text-sm">{member.name}</div><div className="text-xs text-slate-500">{member.role}</div></div>
              </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
        ...
        <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
          <div className="flex items-center gap-4">
            ...
            <div><div className="font-semibold text-slate-800">{associationData.manager.name}</div><div className="text-sm text-slate-500 font-medium">Contact: {associationData.manager.contact}</div></div>
          </div>
          <div className="flex items-center gap-3"><StatusBadge status={associationData.manager.status} /><button className="text-xs text-amber-600 font-semibold hover:underline flex items-center gap-1"><ExternalLink size={12} /> View Details</button></div>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {documents.map((doc, i) => (
            <div key={doc.id || i} className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-lg transition-colors">
              ...
              <div className="flex items-center gap-2"><StatusBadge status={doc.status} /><Download size={16} className="text-slate-400 cursor-pointer hover:text-slate-600" /></div>
            </div>
          ))}
        </div>
      </div>
    ...
    async function loadAll() {
      try {
        const [bm, mt, ci, lc, cp, du, dc] = await Promise.all([
          ...
          fetchFromSupabase('documents'),
        ]);
        setBoardMembers(bm); setMeetings(mt); setComplianceItems(ci);
        setLegalCases(lc); setComplaints(cp); setDelinquentUnits(du); setDocuments(dc);
