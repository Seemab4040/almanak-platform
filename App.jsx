...
// --- LEGAL ---
const Legal = ({ legalCases, setLegalCases, selectedAssociation }) => {
  const [showModal, setShowModal] = useState(false);
  const [saving, setSaving] = useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try { const payload = { ...form, amount: form.amount ? Number(form.amount) : null }; const r = await postToSupabase('legal_cases', { ...payload, association_id: selectedAssociation?.id }); setLegalCases(prev => [...prev, ...r]); setShowModal(false); setForm({ type: 'Debt Collection', description: '', amount: '', status: 'pending', last_action: '', next_step: '' }); }
