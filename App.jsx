      ...
      <div className="bg-white rounded-xl border border-[#e8e4dd] shadow-sm p-5 mt-6">
        ...
        {filteredMeetings.map(m => (
          <div key={m.id} className="mb-4">
            ...
            <div className="flex flex-wrap gap-2 ml-2">{m.agenda && m.agenda.split(',').map((item, idx) => <span key={idx} className="text-xs bg-slate-100 text-[#4a5568] px-2 py-1 rounded">{item.trim()}</span>)}</div>
          </div>
        ))}
      </div>
    </div>
